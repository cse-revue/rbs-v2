...
function toggleSeat(seat) {
    ...
    if (seats[performance][1][seat] == null) {
        ...
        // TODO: Fix weird object generation here
        if (seats[performance][0][seat] > 1 || (seats[performance][0].seats != undefined && seats[performance][0].seats[seat] > 1))
            return;  // you can't unbook other people's seats or paid seats
        else if (seats[performance][0][seat] == 1 || (seats[performance][0].seats != undefined && seats[performance][0].seats[seat] == 1)) {
...
function loadPerformanceUser(perf) {
    a = seats[perf][0];
    if(seats[performance][0].seats != undefined) a = seats[perf][0].seats;
    for (i in a) {
        if(a[i] != 0) {
            setSeat(i, 9);
        } else {
    ...
    for(i = 0; i < bookingseats.length; i++) {
        if(perf == bookingseats[i]['performance']) {
            setSeat(bookingseats[i]['seat'], bookingseats[i]['status'])
        }
    };
    a = seats[perf][1];
    for (i in a) {
