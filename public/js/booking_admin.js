...
function changeDates(date) {
    $('#navigation h2').text(date + "» Select a theatre section");
    $('#status h2').text(date + " » Select a theatre section");
}
...
function showFullTheatre(perf, closed) {
    fulltheatre = true;
    $('#theatre_render').show();
    for (var i in segments) {
        ...
        if(typeof perf !== "undefined") {
            ...
            $($("#segments_" + perf + " a.segmentlink_admin")).last().addClass("selected");
        }
    }
}
...
function toggleModify() {
    if(state == 0) {
        state = 2;
        $("#modifybooking").removeClass("button-selected");
    } else {
        state = 0;
        $("#modifybooking").addClass("button-selected");
    }
}
...
function startNewBooking() {
    ...
    s = document.getElementById('status');
    s.style.display = 'block';
    s.innerHTML = "<h2>" + performances[performance] +" &raquo; Select seats</h2>";
